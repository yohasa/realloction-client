...
// import getCountrLis from '@salesforce/apex/CmbxCountry.GetCountrLis';
// import getCountrStr from '@salesforce/apex/CmbxCountry.GetCountrStr';
import getCountries from '@salesforce/apex/AppRealloc.GetCountries';
import getStore from '@salesforce/apex/AppRealloc.GetStores';
// import GetCountrLis from '@salesforce/apex/CmbxCountry.GetCountrLis';
// import getCountrAgr from '@salesforce/apex/CmbxCountry.GetCountrAgr';
...
async connectedCallback(){
    // console.log('getCountrStr : connected  : ')
    // const res =  await getCountrStr()
    // console.log(res)
    // console.log(JSON.stringify(getCountrLis().toString))
    // console.log(getCountrLis().data)
...
// Object.keys(res2).forEach(key => {
    //     listCountrStr.push(res2[key].Id)
    // })
    // const storeRes = await getStore({lstCrty :idList})
    // console.log('res store : ' + JSON.stringify(storeRes))
    async renderedCallback(){
        
        console.log('getCountrLis : connected 37 : ')
        const res2 = await getCountries();
        console.log('getCountrLis : connected 38 : ')
        console.log(res2)
        console.log('getCountrLis : connected 41 : ')
        console.log(JSON.stringify(res2))
        console.log('getCountrLis : connected  43 : ')
        this.data = JSON.stringify(res2);
    for (var i = 0; i < data.length; i++) {
        await this.idLis.push(this.res2[i].Id);
    }
    
    console.log(this.idLis);
        
        this.idList = await this.data.map(function(item) {
            return item.Id;
        });
    ...
    //     if (data) {
    //         this.picklistInput = data.map(country => ({ label: country.Name, value: country.Id }));
    //     } else if (error) {
    //         console.error(error);
    //     }
    // }
    ...
    handleChange(event) {
        this.picklistInput = event.detail.value;
    }

    get filteredResults() {
        
        console.log('this.picklistInput ', JSON.stringify(this.picklistInput));
        ...
        if (this.valuesVal == undefined) {
            ...
            Object.keys(this.valuesVal).map(profile => {
                this.allValues.push({ Id: this.valuesVal[profile].value, Name: this.valuesVal[profile].label });
            })
